...
# ============================================================
# REGISTRO DE BLUEPRINTS — refatoração estrutural (Fases 1 + 2 + 3)
# ============================================================
...
# _deletar_cloudinary_seguro, limpar_cache_dashboard, _limpar_valor_moeda,
...
# _EXTERNAL_TIMEOUT, COLUNA_ARQUIVO_PARA_BANCO etc.).
...
# ``vendas_bp``, ``documentos_bp``) aplicam ``tenant_required`` automaticamente
# via ``before_request`` — qualquer rota nova adicionada herda a proteção
# sem precisar do decorator manual. ``documentos_bp`` mantém uma allowlist
# explícita de endpoints públicos (token-based) para o bot externo.
#
# Próximas fases extrairão dashboard e caixa. Quando todos os blueprints
# estiverem prontos, moveremos as singletons (db/login_manager/cache/csrf/
# limiter) para um ``extensions.py`` e os helpers para ``services/``.
from routes import (
    auth_bp, master_bp, clientes_bp, produtos_bp, vendas_bp, documentos_bp,
)
