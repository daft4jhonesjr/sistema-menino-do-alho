...
def listar_vendas():
    ...
    # Ordenar pedidos por data (ordenar_por e vencimento são aplicados após o loop que define situacao)
...
# ============================================================
# REGISTRO DE BLUEPRINTS — refatoração estrutural (Fases 1 + 2)
# ============================================================
...
# _safe_db_commit, get_config, limiter, _logs_file, _pos_login_landing,
# query_tenant, empresa_id_atual, registrar_log, _is_ajax,
# _cloudinary_thumb_url, _produto_com_lock, _deletar_cloudinary_seguro,
# limpar_cache_dashboard, _limpar_valor_moeda, _arquivo_imagem_permitido,
# _safe_db_commit, _EXTERNAL_TIMEOUT, COLUNA_ARQUIVO_PARA_BANCO etc.).
# Importar os blueprints somente após esses símbolos estarem definidos
# evita ciclos de import.
#
# Os blueprints ``produtos_bp`` e ``clientes_bp`` aplicam ``tenant_required``
# automaticamente via ``before_request`` — qualquer rota nova adicionada
# nesses módulos herda a proteção sem precisar do decorator manual.
#
# Próximas fases extrairão dashboard, vendas, documentos e caixa.
# Quando todos os blueprints estiverem prontos, moveremos as singletons
# (db/login_manager/cache/csrf/limiter) para um ``extensions.py`` e os
# helpers para ``services/``.
from routes import auth_bp, master_bp, clientes_bp, produtos_bp
